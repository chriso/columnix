...
void test_compression(unsigned char *buffer, enum cx_compression_type type,
                      int level)
{
    size_t compressed_size;
    void *compressed =
        cx_compress(type, level, buffer, BUFFER_SIZE, &compressed_size);
    assert_not_null(compressed);
    assert_size(BUFFER_SIZE, >, compressed_size);
    for (size_t i = 0; i < BUFFER_SIZE; i++)
        buffer[i] = 0;
    assert_true(
        cx_decompress(type, compressed, compressed_size, buffer, BUFFER_SIZE));
    for (size_t i = 0; i < BUFFER_SIZE; i++)
...
{
    test_compression(buffer, CX_COMPRESSION_LZ4, 1);
    test_compression(buffer, CX_COMPRESSION_LZ4, 15);
    return MUNIT_OK;
}

static MunitResult test_zstd(const MunitParameter params[], void *buffer)
{
    test_compression(buffer, CX_COMPRESSION_ZSTD, 1);
    test_compression(buffer, CX_COMPRESSION_ZSTD, 15);
    return MUNIT_OK;
}
