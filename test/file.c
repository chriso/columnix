...
static MunitResult test_read_write(const MunitParameter params[], void *ptr)
{
    ...
    ZCS_FOREACH(zcs_compression_types, compression)
    {
        ...
        for (size_t i = 0; i < COLUMN_COUNT; i++) {
            ...
            assert_true(zcs_writer_add_column(writer, buffer, types[i], 0,
                                              compression, level));
        }
        ...
        for (size_t i = 0; i < ROW_COUNT; i++) {
            ...
            if (i % 12 == 0)
                assert_true(zcs_writer_put_null(writer, 3));
            else
        ...
        for (size_t i = 0; i < COLUMN_COUNT; i++) {
            sprintf(buffer, "column %zu", i);
            const char *name = zcs_reader_column_name(reader, i);
        ...
        for (; zcs_reader_next(reader); position++) {
            ...
            bool null;
            assert_true(zcs_reader_get_null(reader, 3, &null));
            if (position % 12 == 0) {
                assert_true(null);
            } else {
                assert_false(null);
        ...
        struct zcs_predicate *predicate = zcs_predicate_new_and(
            5, zcs_predicate_new_i32_gt(0, 20),
        ...
        assert_not_null(reader);
        assert_size(zcs_reader_column_count(reader), ==, COLUMN_COUNT);
        assert_size(zcs_reader_row_count(reader), ==, 1);
        assert_false(zcs_reader_error(reader));
...
static MunitResult test_empty_columns(const MunitParameter params[], void *ptr)
{
    ...
    ZCS_FOREACH(zcs_compression_types, compression)
    {
        ...
        struct zcs_reader *reader = zcs_reader_new(fixture->temp_file);
        assert_not_null(reader);
        const char *name = zcs_reader_column_name(reader, 0);
        assert_not_null(name);
